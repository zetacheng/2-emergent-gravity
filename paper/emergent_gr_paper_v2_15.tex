% ===================================================================
% Paper 2, v2.15 (2026-07)
% v2.15: RETRACTION. The "minimal pair is radiatively safe" Fierz
%   result -- that the V/A exchange couplings of Eq.(L0) vanish
%   identically -- is withdrawn. It was a gamma5-without-the-i
%   convention slip: L0 uses i*gamma5, and the chirally invariant
%   pair Fierzes to |c_V| = |c_A| = 1/2, not 0. Confirmed by gate
%   P3-FIERZ-01 (github.com/zetacheng/3-vector-sector) and
%   independently against the standard Fierz table.
%   Consequences recorded: (i) G_V IS Fierz-inherited -- the
%   "independently postulated G_V" statement is reversed; the
%   flavor-singlet value is G_omega = -G/N, repulsive;
%   (ii) the minimal-pair restriction loses its Fierz-protection
%   leg in the overlap frame and rests on U(1)_A plus the pairing
%   sign-protection, the latter flagged for re-audit under the
%   corrected convention (same machinery);
%   (iii) the omega route enters the budget: the repulsive channel's
%   HS auxiliary is a healthy massive Proca vector (companion
%   Paper 3 v3.11), and THIS paper owns the induced-gravity budget
%   insertion (registered as an open computation, not a result).
% Companions: Paper 1 v12, Paper 3 v3.11, Paper 4 v6.3, Paper 5 v0.23.
%
% Paper 2, v2.14 (2026-07)
% v2.12: overlap frame qualifier on the criticality cascade.
% v2.13: overlap resolution of the gap-vs-bubble spread;
%        invariant-universal interaction adopted as extension.
% v2.14: Finding-5 outlook and priority list synced to the completed
%        vector pole-level analysis (companion Paper 3, v3.10);
%        abstract qualifier on the vector route added.
% Companion to: Paper 1 (Yukawa/SPARC, v10), Paper 3 (vector channel,
% v3.10), Paper 4 (dark energy, v5.5), Paper 5 (working note, v0.18).
% ===================================================================
...
The one-loop effective potential fixes the condensate amplitude
through the gap equation, while the phase (angular) direction is
flat at this order and is pinned at the real, $CP$-conserving point
only by subleading chiral-symmetry-breaking effects; the substrate
fermions acquire masses of order the lattice cutoff
$\Lambda \sim 1/a$ and are therefore unobservable at low energies,
while the angular direction of the complex condensate hosts a
naturally light pseudo--Goldstone mode whose mass is protected by
the same approximate $U(1)$ symmetry, broken only by the discrete
lattice structure.
...
Two corrections to the vector route are recorded here.  Its
\emph{attractive} branch is now known to require sub-percent
criticality tuning or interacting-level dynamics for a genuine
light state; and, retracting an earlier Fierz result of this paper
(a $\gamma_5$-without-the-$i$ convention slip), the minimal chiral
pair does \emph{not} leave the vector channel uninduced---it
inherits a \emph{repulsive} flavor-singlet coupling
$G_\omega = -G/N$, whose auxiliary the companion vector-sector
paper finds to be a healthy massive Proca-type vector requiring no
tuning.  Inserting that species into the budget of this paper is
identified as the sharpest open computation the paper owns; it is
not performed here, and no claim that the budget closes is made.
...
The general local four-fermion interaction consistent with
hypercubic symmetry and parity assigns an independent coupling to
each Dirac channel,
\begin{equation}
\mathcal{L}_{\rm int} = \sum_{i}
\frac{G_i}{2N}(\bar{\psi}\Gamma_i\psi)(\bar{\psi}\Gamma_i\psi),
\qquad
\Gamma_i \in \{1,\, i\gamma_5,\, \gamma_\mu,\,
\gamma_\mu\gamma_5,\, \sigma_{\mu\nu}\},
\label{eq:Lgen}
\end{equation}
and there is no symmetry that forces the $G_i$ to coincide: the
scalar--pseudoscalar pair and the vector, axial, and tensor
bilinears belong to different chiral and hypercubic
representations.  This paper uses only the condensate sector, the
\emph{minimal chiral pair}
\begin{equation}
\mathcal{L}_0 = \bar{\psi}(i\slashed{\partial})\psi
+ \frac{G}{2N}\Bigl[(\bar{\psi}\psi)^2
+ (\bar{\psi}i\gamma_5\psi)^2\Bigr],
\label{eq:L0}
\end{equation}
where $G \equiv G_\chi > 0$ is the coupling of the chiral
scalar--pseudoscalar multiplet: the two bilinears are the real and
imaginary parts of a single complex order parameter
$\Phi \sim \bar\psi_R\psi_L \sim \sigma + i\pi$, so chiral symmetry
fixes $G_S = G_P = G$ within the pair while leaving
$G_V$, $G_A$, $G_T$ independent.
The $1/N$ prefactor ensures a well-defined large-$N$ limit.
Two computations, recorded at the end of this subsection, justify
the restriction: the equal-coupling extension of
Eq.~\eqref{eq:Lgen} to all channels is \emph{excluded} by the
channel criticality structure, while the minimal pair is
radiatively protected against generating the excluded couplings.
...
Two Fierz-arithmetic results, obtained by machine-verified Dirac
and Grassmann algebra, fix the status of the non-condensate
channels.
\emph{First, the equal-coupling extension is excluded.}
Setting $G_i = G$ for all channels (the Fierz-complete
``master'' interaction, equal by the completeness relation to a
flavor-exchange scalar structure) and tuning $G$ to the
condensate criticality $G_c = 5.93$ places the pseudoscalar,
vector, \emph{and} tensor channels simultaneously above their
bubble criticalities for any light fermion mass: with the
Wilson bubbles of Eq.~\eqref{eq:gcspread} extended
across channels, $1/G_c^{\rm bub} = \Pi_\Gamma(0)$ exceeds
$1/G = 0.169$ for $\Gamma = P, V, T$ at all $m_f a \lesssim 1$,
with subcriticality restored only at $m_f a \gtrsim 2$.
A near-critical condensate with $m_f \ll \Lambda$ is therefore
incompatible with universal channel couplings \emph{in the Wilson
discretization}: the vacuum would reorganize along parity-breaking
and hypercubic-symmetry--breaking directions.
(Frame qualifier, added in v2.12: this cascade is a statement about
the Wilson-defined model.  In the Ginsparg--Wilson (overlap)
discretization---required in any case by the dark-energy
sector---the chirally improved channel bubbles reverse the
ordering: the condensate pair criticality is unified at
$G_c = 1/0.130 = 7.69$ by the exact lattice chiral Ward identity,
while the vector and axial channels sit at
$G_c^{V,A} = 1/0.065 = 15.4$ and the tensor bubble vanishes in the
chiral limit, so the equal-coupling extension is \emph{not}
excluded by the criticality cascade in that frame.  The
restriction to the minimal chiral pair in this paper rests, in the
overlap frame, on the discretization-independent operator results
that follow rather than on the cascade.
\emph{Weakened in v2.15}: of those results, the Fierz protection
of the vector and axial channels has been retracted (below), so
what carries the restriction in the overlap frame is now the
dark-energy sector's $U(1)_A$ requirement together with the
pairing sign-protection---and the latter was produced by the same
machinery as the retracted result and is flagged below for
re-audit.  The restriction should be read, in that frame, as less
securely founded than earlier versions claimed.)
$1/N$ exchange corrections cannot rescue the universal choice:
the Fierz matrix of the complete interaction has vanishing
exchange weight in every channel except the scalar, so the
supercritical channels are not split from the condensate pair at
subleading order.
\emph{Second, the claim that the minimal pair is radiatively safe
is RETRACTED (v2.15).}
Earlier versions reported that the Fierz exchange of
Eq.~\eqref{eq:L0} generates vector and axial couplings that vanish
\emph{identically}, the scalar and pseudoscalar exchange
contributions cancelling channel by channel.  That result is
withdrawn: it was obtained by Fierzing
$(\bar\psi\psi)^2 + (\bar\psi\gamma_5\psi)^2$, whereas
Eq.~\eqref{eq:L0} is built---as it must be, the hermitian
pseudoscalar bilinear being $\bar\psi i\gamma_5\psi$---from
$i\gamma_5$.  The two differ by $(i)^2 = -1$ in the pseudoscalar
square, and that sign is the whole of the cancellation: with the
standard Fierz table the scalar and pseudoscalar squares
contribute $\mp\tfrac{1}{4}$ each to the vector channel, so the
non-invariant combination $s^2 + p^2$ gives zero while the
chirally invariant $s^2 - p^2 = (\bar\psi\psi)^2 +
(\bar\psi i\gamma_5\psi)^2$ gives $|c_V| = |c_A| = \tfrac{1}{2}$.
The correction was established by the companion vector-sector
paper's Fierz gate---re-derived from $16$-dimensional Dirac
completeness, full reconstruction residual
$9\times10^{-16}$, and returning exactly $0$ for $\gamma_5$ and
$+1/2$ for $i\gamma_5$ on one honest sign---and confirmed
independently against the standard table.
\emph{The correct statement is the opposite of the withdrawn one:}
the minimal chiral pair \emph{does} generate vector and axial
couplings by Fierz exchange, at order $G/N$, with flavor-singlet
value $G_\omega = -G/N$---that is, \emph{repulsive}, of Walecka
type.  The equal shifts of $G_S$ and $G_P$ (Fierz rearrangement
preserves the chiral pairing, as it must) and the repulsive tensor
coupling of order $G/N$ are unaffected.
\emph{Third, the particle--particle sector is sign-protected.}
Rearranged into pairing bilinears
$\psi^T C\Gamma\psi$, the minimal pair is \emph{exactly}
\begin{equation}
(\bar\psi\psi)^2 + (\bar\psi i\gamma_5\psi)^2
= -\tfrac{1}{2}\sum_\mu
(\bar\psi\,\tilde{A}_\mu C\,\bar\psi^{\,T})
(\psi^T C\,\tilde{A}_\mu\,\psi),
\qquad \tilde{A}_\mu = i\gamma_\mu\gamma_5,
\label{eq:pairdecomp}
\end{equation}
with identically vanishing scalar- and pseudoscalar-diquark
content (machine-verified exact decomposition in the complete
antisymmetric pairing basis).  The induced axial-diquark coupling
is \emph{negative}, while the corresponding anomalous pair bubble
is \emph{positive}---by the Wilson-exact conjugation identity
$C\,S(-p)^{T}C^{-1} = S(p)$, valid including the Wilson term, the
zero-momentum pair bubble equals the particle--hole bubble of the
same hermitian vertex, numerically $+0.19$.
The pairing gap equation therefore has no solution at any
coupling, any $N$, and any fermion mass: the vacuum is protected
against $\langle\psi\psi\rangle$ condensation by sign, not by
magnitude, and the fermion-number $U(1)$ of the substrate is
unbroken.
\emph{(Flagged, v2.15.)}  This decomposition was produced by the
same machinery as the retracted result above, and
Eq.~\eqref{eq:pairdecomp} is an identity \emph{for the
$i\gamma_5$ pair} specifically.  It is therefore scheduled for
re-audit under the corrected convention before it is relied upon
further.  Two considerations bear on the outcome without settling
it: the companion vector-sector paper's independent
$U(3)$ pairing gauntlet, run with the corrected convention, also
finds the scalar, pseudoscalar and tensor diquark content
identically absent, which is consistent with the structure above;
but the \emph{sign} of the induced axial-diquark coupling---the
whole content of the protection claim---has not been
re-confirmed for the $U(1)$ minimal pair in that convention.  The
claim is reported here as standing but unaudited, not as
established.  Pairing-driven (superconductor-like) phases would
require an additional attractive pairing operator beyond
Eq.~\eqref{eq:L0}; none is induced by it.
\emph{(v2.15.)}  The statement that the composite phenomenology of
the vector sector requires an \emph{independently postulated}
$G_V$, rather than one inherited from $G$ by Fierz rearrangement,
is likewise withdrawn: with the corrected convention $G_V$
\emph{is} inherited, at $-G/N$ in the flavor-singlet channel.
What remains of this subsection's conclusion is narrower: the
minimal chiral pair does not induce the excluded \emph{scalar-type}
condensation channels at $\mathcal{O}(1/N)$, and its induced
vector coupling is repulsive rather than attractive---so no
composite vector \emph{bound state} is inherited, which is a
different and weaker statement than the vanishing coupling
previously claimed.  The consequences of the induced repulsive
channel for this paper's budget are taken up in Finding~5 below.
\emph{(Adopted extension, v2.13.)}  The programme's working
microscopic action is now the classically
$U(1)_A$-preserving \emph{invariant-universal} completion of
Eq.~\eqref{eq:L0}---the chiral pair together with equal-coupling
flavor-singlet vector and axial channels, and, at the
flavor-$U(2)$ level, the $U(2)\times U(2)$ scalar octet---which
is machine-verified to be exactly $U(1)_A$- and
$SU(2)_A$-invariant with \emph{identically vanishing} pairing
content (the strongest vacuum protection encountered: absence
rather than sign or magnitude).  The minimal pair of
Eq.~\eqref{eq:L0} is its condensate sector, so every analysis of
this paper carries over unchanged.
...
The substrate fermions couple to this condensate through a
\emph{chiral} mass term,
\begin{equation}
M = g(\sigma + i\gamma_5\pi) = g\rho\,e^{i\gamma_5\theta},
\label{eq:chiralmass}
\end{equation}
in which the pseudoscalar component $\pi \sim \bar\psi i\gamma_5\psi$
necessarily enters with $\gamma_5$.  Integrating out the fermions at
one loop in Euclidean space then yields the effective potential
\begin{equation}
V_{\mathrm{eff}}(\rho,\theta) = \frac{\rho^2}{2G}
- 2N\int \frac{d^4p_E}{(2\pi)^4}\,
\log\bigl(p_E^2 + g^2\rho^2\bigr),
\label{eq:Veff}
\end{equation}
with the momentum integral regulated at the lattice cutoff
$\Lambda \sim 1/a$.  The phase $\theta$ has \emph{dropped out}: the
Dirac determinant of a chiral mass depends only on the modulus
$|\Phi|^2 = \rho^2$, because the chiral rotation $e^{i\gamma_5\theta}$
assigns opposite phases to the left- and right-handed components,
which cancel pairwise in the closed fermion loop.  Explicitly,
\begin{equation}
\det\bigl(i\slashed{p}_E + g\rho\,e^{i\gamma_5\theta}\bigr)
= \bigl(p_E^2 + g^2\rho^2\bigr)^2 ,
\label{eq:det-theta-indep}
\end{equation}
independent of $\theta$.%
\footnote{An earlier version of this section wrote the integrand as
$\log|p_E^2 + g^2\rho^2 e^{2i\theta}|$, which corresponds to a
\emph{non-chiral} complex mass $g\rho\,e^{i\theta}$ (without
$\gamma_5$) and generates a spurious $\cos 2\theta$ landscape of
amplitude $\mathcal{O}(N\Lambda^4)$.  That form is inconsistent both
with the chiral assignment of the scalar--pseudoscalar pair stated
above and with the ultralight pseudo-Goldstone mode of
Section~\ref{sec:angular}---a Planck-scale angular potential would
give the angular mode a Planck-scale mass.  It is corrected here.}

The radial direction is fixed by the gap equation
$\partial V_{\mathrm{eff}}/\partial\rho = 0$, giving a nonzero
condensate $\langle\rho\rangle = v > 0$ in the broken phase.  At one
loop the \emph{angular} direction is exactly flat: it is a
pseudo-flat direction, the perturbative reflection of the
approximate $U(1)$ chiral phase symmetry $\Phi \to e^{i\alpha}\Phi$.

The phase is therefore fixed not at one loop but by the subleading
effects that explicitly break the chiral $U(1)$: the axial anomaly,
realized on the lattice through the Wilson term and the surviving
doublers, together with the discrete ($Z_M$) structure of the phase
landscape.  These generate a shallow periodic angular potential of
instanton/lattice origin,
\begin{equation}
V_\theta(\theta) \simeq -K\cos(M\theta),
\qquad K \sim \varepsilon\,\Lambda^4,\quad \varepsilon \ll 1,
\label{eq:Vtheta}
\end{equation}
whose minimum lies at the $CP$-conserving point $\theta = 0$.  The
ground state is thus a \emph{real} condensate,
$\langle\Phi\rangle = v > 0$, while the same small coefficient $K$
sets the pseudo-Goldstone mass of Section~\ref{sec:angular}: the
flat one-loop direction and the ultralight angular mode share a
single origin.  (The magnitude of $\varepsilon$ on the $H(4)$
substrate---in particular whether the anomaly contribution is
exponentially instanton-suppressed---is the dedicated computation
left open in Section~\ref{sec:angular}.)

In this chiral parametrization every value of $\theta$ corresponds
to a \emph{Hermitian} mass $g\rho\,e^{i\gamma_5\theta}$; in
particular $\theta = \pi/2$ is the Hermitian pseudoscalar
configuration $g\rho\,i\gamma_5$, degenerate with $\theta = 0$ at one
loop and lifted only by Eq.~\eqref{eq:Vtheta}.  No non-Hermitian
``imaginary'' configuration arises anywhere on the condensate
manifold.  The earlier ``imaginary-vacuum'' hypothesis---a purely
imaginary, non-Hermitian mass $ig\rho$---is therefore excluded
\emph{a fortiori}: it is not a point of the chiral condensate
manifold at all.  We record this explicitly because an earlier
...
and the one-loop flatness established in
Section~\ref{sec:vacuum} is its perturbative signature.
If the $U(1)$ rotation
...
prefactor.
Throughout this paper $G_c$ denotes this gap-equation
(leading-order effective-potential) criticality; it is distinct
from the channel \emph{bubble} criticalities, which in the Wilson
discretization are non-degenerate with it---see the dedicated
paragraph following Finding~5.
...
\emph{(Status note, v2.15.)}  Which vector channel this is
matters, and the answer has changed.  The attractive channel,
whose pole the companion paper audited, supports no
below-threshold state at natural coupling.  The channel the
minimal pair actually induces by Fierz is the \emph{repulsive}
one, $G_\omega = -G/N$ (retraction above), and the companion
paper finds its Hubbard--Stratonovich auxiliary to be a healthy
massive Proca-type vector with no tuning: positive induced
kinetic normalization, non-tachyonic mass, nonvanishing
anomaly-fixed baryon coupling, positive-definite static kernel.
The coefficient derived in this Finding applies to it unchanged.
Whether it \emph{is} a propagating pole at the model's own
coupling, and what it does to the budget arithmetic below, are
open---see the outlook---and nothing in this section should be
read as claiming the budget is thereby closed.
...
\textbf{Gap versus bubble criticality: a Wilson scalar-channel
systematic.}
The critical coupling used above is a leading-order object: it
locates the sign change of the curvature of the effective
potential at the origin (equivalently, the tadpole
self-consistency threshold), which is the physically appropriate
observable for the condensation phase boundary on which the
$m_f \ll \Lambda$ tuning argument and the chain-rule computation
of Finding~4 rely.
A distinct object is the bubble (RPA) criticality of a given
composite channel, $G_c^{\mathrm{bub}} = 1/\Pi_\Gamma(0)$, which
enters only through the next-to-leading-order composite
propagator $D^{-1}(p) = 1/G - \Pi(p)$.
In a chirally symmetric regularization the gap and
scalar--pseudoscalar bubble criticalities degenerate in the
chiral limit; in the Wilson discretization employed here they do
not.
A dedicated evaluation of the channel bubbles on volumes up to
$160^4$ (at $m_f a = 0.05$; conventions anchored to the
vector-channel values of the companion paper, whose published
$\Pi_V(0)$ at $m_f a = 0.2$ and $0.5$ are reproduced to better
than $0.1\%$ and $\Pi_A(0) \simeq -0.19$ is reproduced, and whose
pipeline independently reproduces $I_0$, giving $0.0845$ at
$m a = 0.02$ on $64^4$) yields
\begin{equation}
\frac{1}{T_S(0)} \simeq 7.09, \qquad
\frac{1}{T_P(0)} \simeq 3.02, \qquad
G_c^{\mathrm{gap}} = 5.93,
\label{eq:gcspread}
\end{equation}
a spread from $3.0$ to $7.1$ across definitions that a chiral
regularization would render near-degenerate.
Two effects contribute and are not disentangled here: the
explicit chiral breaking of the Wilson term, which contaminates
the $\bar\psi\psi$ vertex with the momentum-dependent Wilson mass
across the entire Brillouin zone (the same pathology that
obstructs the direct fermionic extraction in Finding~3), and the
genuine $1/N$ difference between the leading-order potential and
the next-to-leading-order propagator.
The lattice scalar bubble moreover fails a continuum consistency
condition: its induced kinetic term carries the wrong sign at
small momenta, whereas the physical chiral-limit scalar and
pseudoscalar bubbles must jointly support a Goldstone pole and a
$\sigma$ pole at $2m_f$ with a shared coupling.
The spread in Eq.~\eqref{eq:gcspread} should therefore be read as
the systematic uncertainty attached to any scalar-channel bubble
quantity in this discretization, and scalar-channel bubble
numbers---including $1/T_S(0)$ above---should not be used as
physical inputs elsewhere in the programme (in particular not in
cross-channel Fierz comparisons) until a chirally clean
extraction (overlap or staggered) exists.
We record one further observation, now resolved by the Fierz
analysis of Section~2.2: within the minimal chiral pair the
pseudoscalar coupling equals the scalar one exactly
($G_P = G_S = G$), so the pseudoscalar channel is populated at
full strength by construction.  In a chirally symmetric
discretization this is not a pathology but the statement that the
condensate selects a direction on the chiral circle
$\sigma + i\pi$---the complex-condensate structure the programme
requires---whereas the Wilson term tilts the circle and splits the
apparent criticalities ($1/T_P(0) \simeq 3.0$ against
$G_c^{\mathrm{gap}} = 5.93$), converting the degenerate angular
direction into an Aoki-type parity-breaking instability of the
discretization.  The operating-point analysis near criticality
should therefore be performed with a chiral discretization, which
is the same requirement already identified for the scalar-channel
extraction.
\emph{(Resolution, v2.13.)}  The chiral (overlap) campaign has
since closed this systematic: with Ginsparg--Wilson fermions and
chirally improved densities, the scalar and pseudoscalar bubbles
are exactly degenerate, the exact lattice chiral Ward identity
makes the gap and bubble criticality definitions coincide, and
the spread collapses to a single number
($G_c = 1/0.130 = 7.69$ in overlap units, $\rho = 1$).  The
Wilson scalar channel's pathology is confirmed as a
discretization artifact (its bubble carries the wrong sign
structure relative to the physical $\Pi_S = -T_S$, which the
overlap value matches); the physical scalar kinetic normalization
is healthy, $Z_S = +0.065 > 0$, and the radial-mode mass is
measured, $m_\sigma/(2m_{\rm dyn}) = 1 - 0.98\,(m_{\rm dyn}a)
+ 0.72\,(m_{\rm dyn}a)^2$, recovering the continuum relation
exactly in the chiral limit.
None of the conclusions of this paper is affected by the spread:
$\beta_B$ is extracted from the condensate-sector heat kernel
(Finding~3), and the induced coupling $\xi_{\mathrm{ind}}$ and the
survival-condition verdict (Finding~4) depend on the gap-equation
phase boundary only, never on a scalar bubble.
...
Two of the priority calculations named in earlier versions are now
complete: the chirally clean scalar-channel extraction has
collapsed the gap-versus-bubble spread of Eq.~\eqref{eq:gcspread}
(overlap resolution, v2.13), and the vector-channel gap and
pole-level analysis has been carried out in the companion
vector-channel paper---with a negative verdict at natural
coupling: the subcritical vector channel supports no
below-threshold bound state, so a light composite vector requires
sub-percent criticality tuning or Bethe--Salpeter-level dynamics.
The remaining priority calculations are the precision lattice test
of $\beta_V/\beta_B = -3$ (Stueckelberg-decomposed
discretization), the interacting-level (Bethe--Salpeter) vector
binding kernel that decides whether a near-critical $G_V$ has a
computed origin, the axial channel's budget coefficient, and the
interacting-level composite--composite light-cone computation on
which the programme's Lorentz-invariance viability rests.
...
\item \textbf{Vacuum}: Real condensate---the one-loop potential
  fixes the amplitude through the gap equation, while the flat
  angular direction is pinned at the real point by subleading
  chiral-symmetry breaking; substrate fermions gapped at
...
\item \textbf{Normalization of the induced Newton constant
  ...
  regime.
  Remaining in-model routes: the \emph{attractive} vector channel
  (conditional, after the companion paper's pole-level analysis, on
  sub-percent criticality tuning or Bethe--Salpeter-level
  dynamics); the \emph{repulsive} vector channel now known to be
  Fierz-inherited from $G$ (v2.15), whose Hubbard--Stratonovich
  auxiliary the companion paper finds to be a healthy massive
  Proca-type vector without tuning, and whose insertion into the
  budget of this paper is uncomputed; non-minimal microscopic
  operators; or $M_{\mathrm{Pl}}^2$ as a defining input.
...
(i) the induced-gravity budget of the \emph{repulsive}
flavor-singlet vector channel.  This is now the sharpest open
computation this paper owns, and v2.15 changes its status from
absent to specified.  The companion vector-sector paper supplies
the inputs---an auxiliary vector with positive induced kinetic
normalization, non-tachyonic mass, nonvanishing anomaly-fixed
baryon coupling and positive-definite static kernel, all
independent of the coupling's value, plus a below-threshold pole
whose existence requires $g_0 \gtrsim 10.6$ and is therefore not
yet established at the model's own $g_0 = G/N$.  What this paper
must supply is the insertion: the species count $n_V$, the
logarithm $L_V$, and the resulting $M_{\mathrm{Pl}}^2$ with
$m_V = m_\omega$, noting that at the representative coupling
$m_\omega \simeq 1.4\,m_f$ rather than sitting in the hierarchy
$m_f \ll m_V \ll \Lambda$ that Eq.~\eqref{eq:budget}'s
derivation assumes, so the substitution is not automatic.  Being
Proca-type, the $\omega$ carries $\beta_V = -3\beta_B$ and
contributes \emph{positively}; whether it contributes
\emph{enough}, and whether it exists as a pole at the model's
coupling, are the two questions.  Completing the precision lattice
test of $\beta_V/\beta_B = -3$ (the sign is established,
Finding~5) and the Bethe--Salpeter-level binding kernel of the
\emph{attractive} channel remain as before, the latter now the
less promising of the two routes;
