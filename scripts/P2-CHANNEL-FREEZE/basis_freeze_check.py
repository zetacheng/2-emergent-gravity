...
def expected_matrix() -> sp.Matrix:
    q = sp.Rational
    return sp.Matrix([[q(1, 4), q(1, 4), q(1, 4), q(1, 4), q(1, 4)],
...
def verify() -> None:
    ...
    assert list(c_block) == ["basis_order", "basis_elements", "conventions", "matrix_rational"]
    assert c_block["basis_order"] == ["S", "P", "V", "A", "T"]
    assert [item["basis_id"] for item in c_block["basis_elements"]] == c_block["basis_order"]
    conventions = c_block["conventions"]
    for key in ("metric_signature", "gamma5_definition", "sigma_definition",
                "dirac_trace_normalization", "un_generator_normalization",
                "grassmann_crossing_sign", "singlet_traceless_order",
                "compound_index_order"):
        assert key in conventions
    assert conventions["grassmann_crossing_sign"] == "-1"
    matrix = sp.Matrix([[sp.sympify(x) for x in row] for row in c_block["matrix_rational"]])
    ...
    assert artifact == {"basis_order": c_block["basis_order"], "matrix_rational": c_block["matrix_rational"]}
    ...
    assert list(d_block) == ["canonical_interaction", "interaction_coordinates",
                             "auxiliary_parameters", "interaction_decomposition",
                             "stated_ranks", "hs_field_families", "kij_registry", "exclusions"]
    ...
    assert canonical(frozen["expression"]) == canonical(companion["canonical_interaction_expression"])
    coords = d_block["interaction_coordinates"]
    assert len(coords) == 1 and coords[0]["coordinate_id"] == "G" and coords[0]["scan_eligible"] is True
    assert all(item["scan_eligible"] is False for item in d_block["auxiliary_parameters"])
    decomposition = d_block["interaction_decomposition"]
    assert [x["family_id"] for x in decomposition] == ["S", "P"]
    reconstructed = "(G/(2*N))*Sum(bilinear(lam(A),Id4)**2+bilinear(lam(A),I*gamma5)**2,(A,0,N**2-1))"
    assert canonical(reconstructed) == canonical(companion["canonical_interaction_expression"])
    ranks = d_block["stated_ranks"]
    assert ranks == {"interaction_coordinate_rank": "1", "fierz_family_rank": "5", "kij_component_count": "16*N**2"}
