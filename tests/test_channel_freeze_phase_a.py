...
def test_channel_freeze_machine_verifier():
    subprocess.run([sys.executable, "scripts/P2-CHANNEL-FREEZE/basis_freeze_check.py"], cwd=ROOT, check=True)
...
def test_channel_freeze_phase_a_frozen_with_hash():
    ...
    digest = hashlib.sha256((ROOT / "derivations/P2-CHANNEL-FREEZE-01_phaseA_freeze.md").read_bytes()).hexdigest()
    assert digest in block


def test_channel_freeze_vector_path_analytic():
    block = (ROOT / "GATES.md").read_text(encoding="utf-8").split("## P2-CHANNEL-FREEZE-01", 1)[1].split("\n## ", 1)[0]
